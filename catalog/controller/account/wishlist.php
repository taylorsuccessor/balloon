...
class ControllerAccountWishList extends Controller {
	public function index() {
		...
		if (isset($this->request->get['remove'])) {
			...
			$this->redirect($this->url->link('account/wishlist'),['Message'=>'success']);
		}
		...
		$num = $this->model_account_wishlist->getTotalWishlist();
		foreach ($results as $result) {
			...
			if ($product_info) {
				...
				if ($this->customer->isLogged() || !$this->config->get('config_customer_price')) {
					...
					$total  = $this->currency->format($unit_price , $this->session->data['currency']);
					//$total = ($price * $q);
				} else {
					$price = false;
				}
